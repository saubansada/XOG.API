...
var oframe = {
    ...
    createBlockTag: function () {
        var html = $.parseHTML("<div id='__BlockScreen__' style='background-color:rgba(0,0,0,0.3); position: fixed; width: 100%; height: 100%; top: 0; left: 0; display:none; z-index:9999999;'><div id='preloader'><div id='loader'></div></div></div>");
    ...
    youtubeGetThumbnail: function (ytId) {
        return "https://img.youtube.com/vi/" + ytId + "/mqdefault.jpg";
    },
    youtubeParseId: function (ytUrl) {
        var regExp = /^.*((youtu.be\/)|(v\/)|(\/u\/\w\/)|(embed\/)|(watch\?))\??v?=?([^#\&\?]*).*/;
        var match = ytUrl.match(regExp);
        return (match && match[7].length == 11) ? match[7] : "";
    },
    getBase64FromImage: function () {
    ...
    getImageData: function (url, callback) {
        var storageName = "Image-" + url;
        if (oframe.canUseLocalStorage() && localStorage[storageName] != null) {
            callback(localStorage["Image-" + url]);
            return;
        }
        ...
        xhr.onload = function () {
            ...
            reader.onloadend = function () {
                localStorage[storageName] = reader.result;
    ...
    canUseLocalStorage: function () {
        ...
        var versionInfo = document.getElementById('localStorageVersion')
        if (versionInfo == null) { return false; }

        var version = versionInfo.value;
        if (localStorage["localStorageVersion"] == version) {
            return true;
        }
        else {
            localStorage["localStorageVersion"] = version;
            return false;
        }
    }
};
