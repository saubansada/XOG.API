...
namespace XOG.Util
{
    public static class Extensions
    {
        ...
        public static string ToRouteKey(this string KeyWordString)
        {
            return KeyWordString.Replace(" ", "_").ToLower();
        }
        ...
        public static bool isNullOrWholeNumber(this object value)
        {
            return value == null || value is int || value is long || value is short || value is byte;
        }

        public static int NullReverse(this object value)
        {
        ...
        public static RouteValueDictionary ToRouteValueDictionary(this object obj, params string[] routeValues)
        {
            return obj.ToRouteValueDictionary("", routeValues);
        }

        public static RouteValueDictionary ToRouteValueDictionary(this object obj, string defRoute = "", params string[] routeValues)
        {
            RouteValueDictionary dictionary = new RouteValueDictionary(obj);

            if (!string.IsNullOrWhiteSpace(defRoute))
            {
                dictionary.Add(defRoute.Split(':')[0], defRoute.Split(':')[1]);
            }

            foreach (string routeValue in routeValues)
            {
                dictionary.Add(routeValue.Split(':')[0], routeValue.Split(':')[1]);
            }
            return dictionary;
        }
    }
